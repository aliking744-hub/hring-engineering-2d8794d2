...
async def update_onboarding_progress(
    ...
    *,
    ...
    if row is None:
        raise DevelopmentNotFoundError("Onboarding plan was not found")
    completed = [item for item in payload.completed_task_indexes if item < payload.total_tasks]
    score = round(len(completed) * 100 / payload.total_tasks)
    row.progress = {
    ...
    row.score = score
    if payload.finalize:
        row.status = "completed" if score >= 50 else "failed"
